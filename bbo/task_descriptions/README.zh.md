...
- `bbo/task_descriptions/bbob_10d/`：官方 COCO/BBOB 24 函数套件共用、不会泄露函数身份的描述包
...
- **dbtune surrogate：**当前 active dbtune 描述只包括 6 个目录：`knob_surrogate_sysbench_5/`、`knob_surrogate_sysbench_all/`、`knob_surrogate_job_5/`、`knob_surrogate_job_all/`、`knob_surrogate_pg_5/`、`knob_surrogate_pg_20/`。它们说明 sklearn 代理设定。数据与实现已并入 `bbo/tasks/dbtune/`（`assets/`、进程内任务、可选 `docker_surrogate/`）。文内路径应写 `bbo/tasks/dbtune/...`。
- **legacy dbtune MariaDB / sysbench：**`knob_http_mariadb_sysbench_*` 目录仅为历史溯源保留，不属于当前 active dbtune task set。
